...
export async function load({ fetch, url }) {
  ...
  const performancesCategory = categories.find(
    (category) => category.slug === "performances"
  );

  if (!performancesCategory) {
    return {
      performances: [],
  ...
  const postsResponse = await fetch(
    `${PUBLIC_WP_API_URL}/posts?_embed&per_page=100&categories=${performancesCategory.id}`
  );

  const posts = await postsResponse.json();

  const performances = posts.map((post) => {
